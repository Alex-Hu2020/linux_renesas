...
enum { LT8912_AUDIO_NONE, LT8912_AUDIO_SPDIF, LT8912_AUDIO_I2S };
...
enum { H_act = 0, V_act, H_tol, V_tol, H_bp, H_sync, V_sync, V_bp };
...
void lt8912_lvds_scaler_config(struct lt8912 *lt) {
  ...
  u32 ColorDeepth;
  u8 temp;
  u32 lvds_hactive, lvds_hfp, lvds_hsync, lvds_hbp, lvds_vfp, lvds_vsync,
      lvds_vbp, lvds_htotal, lvds_vtotal, lvds_vactive;
  u32 lvds_clock;
  ...
  regmap_write(lt->regmap[0], 0x8b,
               (u8)((lvds_hbp / 256) * 0x80 + (lvds_vsync % 256)));
  regmap_write(lt->regmap[0], 0x8c, (u8)(lvds_hactive % 256));
  regmap_write(lt->regmap[0], 0x8d, (u8)(lvds_vactive % 256));
  regmap_write(lt->regmap[0], 0x8e,
               (u8)((lvds_vactive / 256) * 0x10 + (lvds_hactive / 256)));

  Core_PLL_Ratio.Temp32 =
      ((100000 * (hactive - 1)) / (lvds_hactive - 1)) * 4096;
  ...
  Core_PLL_Ratio.Temp32 =
      ((100000 * (vactive - 1)) / (lvds_vactive - 1)) * 4096;
  Core_PLL_Ratio.Temp32 = (Core_PLL_Ratio.Temp32 + 50000) / 100000;
  temp = (u8)(Core_PLL_Ratio.Temp32 & 0xff);
  ...
  regmap_write(lt->regmap[0], 0xa8,
               _VesaJeidaMode + _DE_Sync_mode + ColorDeepth);
...
void lt8912_lvds_bypass_config(struct lt8912 *lt) {
  ...
  regmap_write(lt->regmap[0], 0x51,
               0x2d); // Pix_clk as reference,second order passive LPF PLL
...
static void lt8912_avi_config(struct lt8912 *lt) {
  ...
  // 0x43寄存器是checksums，改变了0x45或者0x47
  // 寄存器的值，0x43寄存器的值也要跟着变，
  // 0x43，0x44，0x45，0x47四个寄存器值的总和是0x6F

  //	HDMI_VIC = 0x04; // 720P 60; Corresponding to the resolution to be
  //output
   ...
   *********************************************************************************/
  AVI_PB0 = ((AVI_PB1 + AVI_PB2 + HDMI_VIC) <= 0x6f)
                ? (0x6f - AVI_PB1 - AVI_PB2 - HDMI_VIC)
                : (0x16f - AVI_PB1 - AVI_PB2 - HDMI_VIC);

  // regmap_write(lt->regmap[2], 0x43, AVI_PB0 );    //avi packet checksum
  // ,avi_pb0 regmap_write(lt->regmap[2], 0x44, AVI_PB1 );    //avi packet
  // output RGB 0x10 regmap_write(lt->regmap[2], 0x45, AVI_PB2 );    //0x19:4:3
  // ; 0x2A : 16:9 regmap_write(lt->regmap[2], 0x47, HDMI_VIC );   //VIC(as
  // below);1080P60 : 0x10

  regmap_write(lt->regmap[2], 0x3e, 0x0A);
  regmap_write(lt->regmap[2], 0x43, 0x46 - HDMI_VIC);
  regmap_write(lt->regmap[2], 0x44, 0x10);
  regmap_write(lt->regmap[2], 0x45,
               0x2A); // 0x19 4:3 0x2A:16:9    0x43,0x44,0x45 0x47 the sum of
                      // the four register value is 0x6f
  regmap_write(lt->regmap[2], 0x47, 0x00 + HDMI_VIC);
}

static void lt8912_audio_config(struct lt8912 *lt) {
  // regmap_write(lt->regmap[0], 0xb2, 0x01); // 0x01:HDMI; 0x00: DVI
  // lt->sink_is_hdmi
  ...
  // 		regmap_write(lt->regmap[2], 0x34, 0xe2); //sclk = 64fs, 0xd2; sclk =
  // 32fs, 0xe2. 		break;
  // }
}
...
static void lt8912_init(struct lt8912 *lt) {
  ...
  regmap_write(lt->regmap[1], 0x13,
               lt->num_dsi_lanes %
                   0x04); // 00 4 lane  // 01 lane // 02 2lane //03 3 lane
  ...
  if (lt->lvds_ctr) {
    ...
    Core_PLL_Ratio.Temp32 =
        (Core_PLL_Ratio.Temp32 - (Core_PLL_Ratio.Temp32 / 1000) * 1000) *
        16384;                                            // E2*1000
...
static int lt8912_connector_get_modes(struct drm_connector *connector) {
  ...
  ret = drm_display_info_set_bus_formats(&connector->display_info, &bus_format,
                                         1);
...
static int lt8912_bridge_attach(struct drm_bridge *bridge,
  ...
  if (!(flags & DRM_BRIDGE_ATTACH_NO_CONNECTOR)) {
    ret = drm_connector_init(bridge->dev, connector, &lt8912_connector_funcs,
...
static int lt8912_i2c_init(struct lt8912 *lt, struct i2c_client *client) {
...
static int lt8912_probe(struct i2c_client *i2c,
  ...
  ret = devm_request_threaded_irq(dev, lt->irq, NULL, lt8912_hpd_irq_thread,
...
static const struct i2c_device_id lt8912_i2c_ids[] = {{"lt8912", 0}, {}};
